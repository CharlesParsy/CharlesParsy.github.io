...
export function checkAnswer(selectedBtn, choice, correct) {
    ...
    solutionEl.style.fontStyle = "italic";
    solutionEl.style.marginTop = "8px";
    solutionEl.textContent = q.solution ? `💡 Explication : ${q.solution}` : "";
    document.getElementById("result").appendChild(solutionEl);
...
export async function addQuestionWithCategory(eventOrData) {

    let categorie, intitule, reponse, propositions, solution;

    // 🔹 CAS 1 : ajout depuis le formulaire
    ...
    // 🔹 CAS 2 : ajout depuis Excel
    else {

        categorie = eventOrData.categorie;
        intitule = eventOrData.intitule;
        reponse = eventOrData.reponse;
        propositions = eventOrData.propositions;
        solution = eventOrData.solution || "";
    }
...
// 🔹 Supprime toutes les questions d’une collection
export async function deleteCollection(cat) {
    const colRef = collection(db, cat);
    const snapshot = await getDocs(colRef);
    for (const docSnap of snapshot.docs) {
        await deleteDoc(doc(db, cat, docSnap.id));
    }
}

// 🔹 Supprime le nom de la catégorie dans la collection 'categories/liste'
export async function removeCategoryName(cat) {
    const catRef = doc(db, "categories", "liste");
    const snap = await getDoc(catRef);
    if (snap.exists()) {
        const noms = snap.data().noms || [];
        const newNoms = noms.filter(c => c !== cat);
        await updateDoc(catRef, { noms: newNoms });
    }
}

// 🔹 Fonction principale pour supprimer une catégorie
